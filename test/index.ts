...
describe(CollectionConfig.contractName, function () {
  ...
  it("Check initial data", async function () {
    expect(await contract.name()).to.equal(CollectionConfig.contractName);
    ...
    expect(await contract.maxFree()).to.equal(CollectionConfig.maxFree);
    expect(await contract.maxperAddressPublicMint()).to.equal(
      CollectionConfig.whitelistSale.maxperAddressPublicMint
    );
    expect(await contract.maxperAddressFreeLimit()).to.equal(
    ...
    await expect(contract.tokenURI(1)).to.be.revertedWith(
      "URI query nonexistant token"
    );
  });

  it("Before any sale", async function () {
    // Whitelisted user cannot mint the contract in a paused state
    await expect(
      contract
    ...
    ).to.be.revertedWith("Whitelist sale not enabled!");
    ...
    // But not over the maxperAddressPublicMint
    await expect(
      contract.mintForAddress(
        await (await contract.maxperAddressPublicMint()).add(6), // the actual maxMint is 5
  ...
  it("Whitelist sale", async function () {
    ...
    // sending an invalid minting amount
    await expect(
      contract
        .connect(whitelistedUser)
        .whitelistMint(
          await (await contract.maxperAddressPublicMint()).add(6),
          merkleTree.getHexProof(keccak256(await whitelistedUser.getAddress())),
          {
            value: getPrice(
              SaleType.WHITELIST,
              await (await contract.maxperAddressPublicMint()).add(6).toNumber()
            ),
          }
        )
    ).to.be.revertedWith("Can't mint more than max mint");
    ...
    expect(await contract.balanceOf(await holder.getAddress())).to.be.equal(0);
    expect(
      await contract.balanceOf(await externalUser.getAddress())
    ).to.be.equal(0);
  });

  it("Pre sale (same as public state)", async function () {
    ...
    // sending invalid mint amout
    await expect(
      contract
        .connect(whitelistedUser)
        .mint(await (await contract.maxperAddressPublicMint()).add(6), {
          value: getPrice(
            SaleType.PRE_SALE,
            await (await contract.maxperAddressPublicMint()).add(6).toNumber()
          ),
    ...
    await contract
      .connect(holder)
      .mintFree(1, { value: getPrice(SaleType.FREE, 1) });

    // sending invalid free mint amount
    await expect(
      contract
        .connect(whitelistedUser)
        .mintFree(await (await contract.maxperAddressFreeLimit()).add(1), {
          value: getPrice(
            SaleType.FREE,
            await (await contract.maxperAddressFreeLimit()).add(1).toNumber()
          ),
        })
    ).to.be.revertedWith("max free NFT per address exceeded");
  ...
  it("Owner only functions", async function () {
    ...
    await expect(
      contract.connect(externalUser).setMaxperAddressPublicMint(9999)
    ).to.be.revertedWith("Ownable: caller is not the owner");

    await expect(
      contract.connect(externalUser).setMaxperAddressFreeMint(9999)
    ).to.be.revertedWith("Ownable: caller is not the owner");
    ...
    // await expect(
    //   contract.connect(externalUser).withdrawAny(utils.parseEther("10"))
    // ).to.be.revertedWith("Ownable: caller is not the owner");
  });
  ...
  it("Supply checks (long)", async function () {
    ...
    await contract.setPaused(false);
    await contract.setMaxperAddressPublicMint(maxMintAmountPerTx);
    ...
    expect(await contract.totalSupply()).to.be.equal(expectedTotalSupply);
